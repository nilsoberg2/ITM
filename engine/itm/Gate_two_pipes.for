...
! 02110-1301, USA.
	...
	do j = 1, NodeNS(R)
     ...
     &		        A1(j),Q1(j),IDf1(j))		
		    elseif (Nodetype(R,j) == 2)then !outflowing
			    ...
			    write(98,*),'subrout. Gate_two_pipes'
			    call endprog
		    endif
		    u1(j) = Q1(j)/A1(j)	
		    ...
		    elseif(IDf1(j) ==1)then
			    ...
			    write(99,*),'subrout. Gate_two_pipes'
			    call endprog
		    endif    
	enddo	
	
	!To avoid error in compuatations when the flow depth is zero
	do j = 1, NodeNS(R)
 		    k = NodeID(R,j)
	      if(IDf1(j) == 0)then
			    if (y1(j) < ydry(k))then
	...
	elseIf(y1(1)<= (1d0+0.01)*h_gate_m(R) .and. 
     &	    y1(2)<= (1d0+0.01)*h_gate_m(R))then 
	      call junct2pipes_same_diam(R)
		    goto 20
      elseif (y1(1)<= 1.1*ydry(k) .and. 
     &	    y1(2)<= 1.1*ydry(k))then 
...
10	do j = 1, NodeNS(R)
	...
	yres_jun_old(R) = min(y1(1),y1(2)) !Minimum of water depths at the gate boundary
	goto 20
...
12	parint1000 = 0 
 ...
 20   continue
	...
	do j = 1, NodeNS(R)		
		    ...
		    if (abs(x(p+1)) > 10000d0)then
		        x(p+1) = 10000d0* SIGN(1d0,x(p+1))
                  parint1000 = 1 
		    endif		    
     ...
     &	    x(3)<= (1d0+Tol_int_10_2)*h_gate_m(R))then 
	      ...
	      elseif (Nodetype(R,1)+Nodetype(R,2) == 3)then !outflowing
	          ...
	          write(98,*),'Problem with Nodetype. Nodetype n.e. 1 and 2'
		        write(98,*),'Check routine gate_solver'
		        call endprog
	      endif
	      goto 107
