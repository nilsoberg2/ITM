...
!    to FCN is at least MAXFEV by the end of an iteration.
!
!    Input, double precision EPSFCN, is used in determining a suitable step length for 
...
!    2, scaling is specified by the input DIAG vector. 
!
!    Input, double precision FACTOR, determines the initial step bound.  This bound is 
...
!    Output, integer NFEV, the number of calls to FCN.
!
!    Output, double precision FJAC(LDFJAC,N), an M by N array.  The upper N by N submatrix
...
!    Input, double precision QTB(N), the first N elements of the vector Q'*B.
!
!    Input, double precision DELTA, an upper bound on the euclidean norm of D*X.  
...
!
!    Output, double precision X(N), the least squares solution of the system A*X = B, 
!    sqrt(PAR)*D*X = 0, for the output value of PAR.
!
!    Output, double precision SDIAG(N), the diagonal elements of the upper triangular 
!    matrix S.
!
  implicit none
...
!    squares.  FTOL should be nonnegative.
!
!    Input, double precision XTOL.  Termination occurs when the relative error between 
!    two consecutive iterates is at most XTOL.  XTOL should be nonnegative.
!
!    Input, double precision GTOL. termination occurs when the cosine of the angle 
...
!    to FCN with IFLAG = 1 is at least MAXFEV by the end of an iteration.
!
!    Input/output, double precision DIAG(N).  If MODE = 1, then DIAG is set internally.
...
!    LDFJAC must be at least N.
!
!    Input, double precision TOL. Termination occurs when the algorithm estimates 
...
!
!    Input/output, double precision V(N).  On input, V must contain the vector V.
