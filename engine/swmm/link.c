...
//
{
    ...
    Link[j].cLossAvg     = 0.0;//x[2];
    Link[j].hasFlapGate  = 0;//k;
    return 0;
}
...
//
{
	...
	v = Link[j].velocity; //link_getVelocity(j, q, y);
	fr = Link[j].froude; //link_getFroude(j, v, y);
    c = 0.0;
...
////////////////////////////////////////////////////////////
   ...
   // --- parse optional parameters
    ...
    ////BEGIN ITM CODE
    Link[j].initDepth = MISSING;
    Link[j].initDepthCode = INIDEP_CONSTANT;
    if ( ntoks >= 10 )
    {
        m = findmatch(tok[8], InitialDepthWords);
        if (m >= 0)
        {
            if (m == INIDEP_CRITICAL)
                Link[j].initDepthCode = INIDEP_CRITICAL;
            else if (m == INIDEP_NORMAL)
                Link[j].initDepthCode = INIDEP_NORMAL;
            else if (m == INIDEP_CONSTANT)
            {
                Link[j].initDepthCode = INIDEP_CONSTANT;
                if ( !getDouble(tok[9], &Link[j].initDepth) )
...
//
{
    ...
    // In ITM, negative offsets are allowed.
...
//
{
    ...
    // --- compute equivalent orifice coeff. (for CMS flow units)
...
{                                                                              //(5.0.011 - LR)
    ...
    {                                                                          //(5.0.011 - LR)
        ...
        // --- compute equivalent orifice coeff. (for CMS flow units)          //(5.0.011 - LR)
