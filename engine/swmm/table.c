...
int table_getCurrentEntry(TTable *table, double *x, double *y)
//
//  Input:   table = pointer to a TTable structure
//  Output:  x = x-value of current table entry
//           y = y-value of current table entry
//           returns TRUE if successful, FALSE if not
//  Purpose: retrieves the current x/y entry in a table.
//
{
    TTableEntry *entry;
    entry = table->thisEntry;
    if ( entry )
    {
        *x = entry->x;
        *y = entry->y;
...
int table_findEntry(TTable *table, double x)
//
//  Input:   table = pointer to a TTable structure
//           x = an x-value
//  Output:  returns true/false; status of find
//  Purpose: sets the current pointer of the table iterator to be the entry
//           whose x-value is less than or equal to x
//
//  NOTE: if x is below the first table entry, then the first entry is
//        returned; if x is above the last entry, then the last entry is
//        returned.
//
{
    double x1,y1,x2,y2;
	...
	table_getLastEntry(table, &x2, &y2);
	if (x >= x2)
		return TRUE;
    table_getFirstEntry(table, &x1, &y1);
    if ( x <= x1 )
...
#define NORMALIZE_TIME(x) ((x - offset)*86400.0)

double  table_getAreaFromSimStart(TTable* table, double x)
//
//  Input:   table = pointer to a TTable structure
//           x = an value
//  Output:  returns area value
//  Purpose: finds area under a tabulated curve from simulation
//           start to x; requires that table's x values be
...
//
{
    ...
    double dx = 0.0, dy = 0.0;
    double a, s = 0.0;
	double offset = StartDateTime;

	if (! table_findEntry(table, offset) )
		return 0.0; //ERROR?
	table_getCurrentEntry(table, &x1, &y1);
...
double  table_getVolume(TTable* table, double x)
//
//  Input:   table = pointer to a TTable structure
//           x = an value
//  Output:  returns volume value
//
//           requires that table's x values be non-negative
//           and non-decreasing.
{
    table_getFirstEntry(table, &(table->x1), &(table->y1));
    //table_getLastEntry(table, &(table->x2), &(table->y2));
    table_getNextEntry(table, &(table->x2), &(table->y2));

    return table_tseriesLookup(table, x, 0);
}
