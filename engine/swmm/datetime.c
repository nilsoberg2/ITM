...
void datetime_decodeTimeHiRes(DateTime time, int* h, int* m, double* s)
...
int datetime_strToTimeHiRes(char* s, DateTime* t)

//  Input:   s = time as string
//  Output:  t = encoded time (hi res),
//           returns 1 if conversion successful, 0 if not
//  Purpose: converts a string time to a DateTime value.

{
	...
	double sec = 0;
    *t = 0.0;
    n = sscanf(s, "%d:%d:%f", &hr, &min, &sec);
    if ( n == 0 ) return 0;
    *t = datetime_encodeTime(hr, min, (int)sec);
	// Add the fractional seconds
	*t = ((*t * SecsPerDay) - (int)sec + sec) / SecsPerDay;
    if ( (hr >= 0) && (min >= 0) && (sec >= 0) ) return 1;
    else return 0;
}
