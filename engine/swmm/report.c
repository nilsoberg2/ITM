...
//
{
    ...
    if ( Frpt.file )
    {
		DEBUGWRITE("start report write");
        fprintf(Frpt.file, FMT20, ctime(&SysTime));
		DEBUGWRITE("after first write");
...
//
{
    ...
    datetime_timeToStr(datetime_encodeTime(0, 0, ReportStep/1000), str);
    fprintf(Frpt.file, "\n  Report Time Step ......... %s.%03d", str, (ReportStep%1000));
    //if ( Nobjects[SUBCATCH] > 0 )
    //{
    ...
    if ( Nobjects[LINK] > 0 )
    {
        fprintf(Frpt.file, "\n  Maximum Time Step ........ %.2f sec", RouteStep);
    }
    WRITE("");
}
