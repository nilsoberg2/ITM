...
//
{
	...
	retcode = itm_read_hotstart_swmm(&Fhotstart1);
    ...
    int   nPollut;
    int   flowUnits;
    char  fileStamp[] = "ITM-HOTSTART";
    char  fStamp[] = "ITM-HOTSTART";

	// Read ITM hotstart file
	retcode = itm_read_hotstart_swmm(Fhotstart1);

	if (retcode || ErrorCode)
		return FALSE;
	
	// Now read SWMM hotstart file
...
//
{
	...
	rescode = itm_open_hotstart2_swmm(&Fhotstart2);
...
//
{
	itm_write_hotstart_swmm(&Fhotstart2);
