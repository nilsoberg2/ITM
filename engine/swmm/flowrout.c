...
//
{
    ...
    for ( j = 0; j < Nobjects[NODE]; j++ )
    {
        switch ( Node[j].type )
        {
          ...
          // --- outfalls cannot have any outlet links. This was commented by Arturo. 
          //case OUTFALL:
          //  if ( Node[j].degree > 0 )
          //  {
...
//
{
    ...
    for ( i=0; i<Nobjects[NODE]; i++ )
    {
        ...
        {
            //if ( Node[i].degree + (int)Node[i].inflow > 1 ) !Next four line were commented because it fails when you have a single pipe
            //{ 
